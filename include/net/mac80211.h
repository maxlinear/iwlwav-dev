 ...
 *	status changed.
 * @BSS_CHANGED_AP_ISOLATE: AP isolation setting changed for this AP interface.
 *
 */
enum ieee80211_bss_change {
	...
	/* The bit flag value must be unsigned to avoid sign bit
	 * extension on 64-bits opertaions.
	 * Therefore, the value (1<<31) is replaced with BIT(31).
	 */
	BSS_CHANGED_UNSOL_BCAST_PROBE_RESP = BIT(31),
 ...
 *	the current band.
 * @ap_isolate: do not forward packets between connected stations, relevent
 *	only on AP interfaces for IEEE80211_HW_DATA_OFFLOAD drivers.
 ...
 *	hardware or driver)
 * @RX_FLAG_CTRL_PORT_PROTO : this frame is a port control
 *  protocol frame (e.g. EAP)
 */
 ...
 * @snr_db: snr for received frame in dBm
 * @noise: noise level when receiving this frame in dBm.
 ...
 * @ml_sta_info: Multi-Link STA info
 */
 ...
 *
 * @IEEE80211_HW_DATA_OFFLOAD: Hardware does data packets handling by itself.
 *	Data packets will be passed between network stack and driver directly.
 *	This must include: header conversion, aggregation, ACK handling, etc.
 *	Drivers using this feature must either support hardware connection
 *	monitoring (%IEEE80211_HW_CONNECTION_MONITOR) or implement
 *	get_connection_alive callback.
 *
 ...
 *	offload
 * @IEEE80211_HW_SUPPORTS_PROXY_ARP: Hardware supports Proxy ARP.
 *
 * @IEEE80211_HW_SUPPORTS_VENDOR_VHT: Hardware supports vendor VHT on 2.4GHz
 *
 * @IEEE80211_HW_SUPPORTS_SSID_LIST: Hardware supports SSID List
 * as extend capabilities.
 *
 * @IEEE80211_HW_SUPPORTS_WNM_NOTIF: Hardware supports WNM Notification
 *	extend capabilities.
 *
 * @IEEE80211_HW_SUPPORTS_OP_MODE_NOTIF: Hardware supports Operating mode
 *	notification extend capabilities.
 *
 ...
 *
 * The set_default_multicast_key() call sets the default (multicast) key index,
 *  useful for WPA.
 *  This is part of the DATA_OFFLOAD feature if driver/hw handles data itself.
 *
 ...
 *  If hardware data offload is enabled (%IEEE80211_HW_DATA_OFFLOAD),
 *  only management transmitted frames are handled by this handler.
 *	Must be atomic.
 *
 * @data_tx: handler that being called for each transmitted data frame.
 *  skb contains the buffer starting from the IEEE 802.3 header.
 *  This callback should only be used when the driver registered
 *  hardware data offload (%IEEE80211_HW_DATA_OFFLOAD).
 *  This handler should, preferably, never fail and stop
 ...
 *
 * @set_default_multicast_key: Set the default (multicast) key index, useful
 *  for WPA. This is part of the DATA_OFFLOAD feature if driver/hw
 *  handles data itself.
 *
 ...
 *
 * @prepare_cac_start: ask the driver to prepare for CAC.
 *
 * @notify_cac_finished: nofity the driver that CAC has ended, and whether
 * 	it was succesful, or canceled.
 *
 ...
 *	This callback may sleep.
 *
 * @get_connection_alive: Called to inquire the driver whether or not
 *      data have been transfered from the last invoke.
 *      This may be implemented by drivers that uses data offloading
 *      (%IEEE80211_HW_DATA_OFFLOAD) and wish to use mac80211's connection
 ...
 * @driver_alloc_netdev: Get a netdev allocated by the driver.
 * This is designed to support iwlwav DC_DP which request net_device on init.
 * @driver_free_netdev_allowed: Is the driver allowing us to free the net_dev.
 * This is again done to support iwlwav DC_DP, which needs the net_dev to
 * not be freed until rmmod.
