			      ...
			      struct cfg80211_ap_settings *params)
{
	...
	if (params->eht_cap) {
		...
		sdata->vif.bss_conf.eht_su_beamformer = false;
		sdata->vif.bss_conf.eht_su_beamformee = false;
		sdata->vif.bss_conf.eht_mu_beamformer = false;
	}
				...
				struct station_parameters *params)
{
	...
	if (params->he_6ghz_capa)
		sta->sta.he_6ghz_capa.capa = params->he_6ghz_capa->capa;
		...
		list_for_each_entry(sdata_iter, &local->interfaces, list) {
			if (sdata_iter->wdev.cac_started) {
				...
				if (wiphy->allow_scan_during_cac) {
					...
					ieee80211_vif_release_channel(sdata_iter);
...
ieee80211_color_change_bss_config_notify(struct ieee80211_sub_if_data *sdata,
					 u8 color, bool enable, u32 changed)
{
...
static int ieee80211_color_change_finalize(struct ieee80211_sub_if_data *sdata)
{
	...
	if (err) {
		//TODO-COLOR: Can be rare, But How do we sync hapd,Driver/FW from here?
		cfg80211_color_change_aborted_notify(sdata->dev);
		return err;
	}
...
void ieee80211_color_coll_notify_work(struct work_struct *work)
{
	...
	ieeee80211_obss_color_collision_notify(&sdata->vif,
		sdata->color_coll_notify.color_bitmap,
		sdata->color_coll_notify.bssid);
				       ...
				       u64 color_bitmap, u8 * bssid)
{
	...
	cfg80211_obss_color_collision_notify(sdata->dev, color_bitmap, bssid);
}
		       ...
		       struct cfg80211_color_change_settings *params)
{
	...
	sdata->vif.color_switch_time = params->switch_time;
	sdata->vif.color_change_cntdown = params->count;
