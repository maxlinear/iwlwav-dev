...
skip_rates:
	 ...
	 * For 6GHz there is no HT/VHT support, but HE is supported */
	if (sband->band != NL80211_BAND_6GHZ) {
						...
						struct ieee802_11_elems *elems)
{
	...
	if (sdata->vif.bss_conf.he_bss_color.color != bss_color) {
		sdata->vif.color_change_color = bss_color;
		sdata->vif.color_change_cntdown = count;
		sdata->vif.color_switch_time = (count * sdata->vif.bss_conf.beacon_int);
...
static int ieee80211_sta_process_color_switch(struct ieee80211_sub_if_data *sdata,
				 struct ieee802_11_elems *elems)
{
	if (sdata->vif.color_change_active || !elems->cca_ie) {
		return 0;
	}
				...
				int shift)
{
	...
	for (i = 0; i < supp_rates_len; i++) {
		...
		/*
		 * Skip HT, VHT, HE, EHT and SAE H2E only BSS membership selectors
					 ...
					 size_t len)
{
	...
	if (status_code != WLAN_STATUS_SUCCESS) {
		...
		if (!ieee80211_assoc_success(sdata, cbss, mgmt, len)) {
					 ...
					 struct sk_buff *skb)
{
	...
	elems = ieee802_11_parse_elems_crc(mgmt->u.probe_resp.variable, len - baselen,
				 false, sdata->wdev.vendor_events_filter,
				     ...
				     struct ieee80211_rx_status *rx_status)
{
	...
	elems = ieee802_11_parse_elems_crc(mgmt->u.beacon.variable,
		len - baselen, false, sdata->wdev.vendor_events_filter,
		sdata->wdev.vendor_events_filter_len, 0, 0, NULL, NULL);
	if (!elems)
		return;
	...
	if (ifmgd->assoc_data && ifmgd->assoc_data->need_beacon &&
		...
		kfree(elems);
		elems = ieee802_11_parse_elems(variable,
				       len - baselen, false,
				       bssid,
	...
	if (bss_conf->he_support && sdata->vif.color_change_active == false) {
		u8 color, enabled;
		/* CCA IE look up and color change */
		changed |= ieee80211_sta_process_color_switch(sdata,
							      elems);
				  ...
				  struct cfg80211_bss *cbss)
{
	...
	/* disable HT/VHT/HE/EHT if we don't support them */
			...
			struct cfg80211_assoc_request *req)
{
	...
	if (!bss->wmm_used) {
		...
		ifmgd->flags |= IEEE80211_STA_DISABLE_EHT;
		netdev_info(sdata->dev,
			    "disabling HT/VHT/HE/EHT as WMM/QoS is not supported by the AP\n");
	}
