					  ...
					  struct cfg80211_chan_def *chandef)
{
	...
	if (!wiphy_ext_feature_isset(&rdev->wiphy, NL80211_EXT_FEATURE_RADAR_BACKGROUND))
		return -EOPNOTSUPP;
	...
	/* Get CAC time */
	if (rdev->wiphy.cac_time_debug >= 0 && rdev->wiphy.cac_time_debug <= IEEE80211_DFS_MAX_CAC_TIME_MS)
		cac_time_ms = rdev->wiphy.cac_time_debug;
	else {
