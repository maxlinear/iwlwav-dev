...
static int nl80211_calculate_ap_params(struct cfg80211_ap_settings *params)
{
	...
	const u8 *rates;
	const u8 *cap;
	const struct element *cap_element;
	...
	cap_element = cfg80211_find_ext_elem(WLAN_EID_EXT_EHT_CAPABILITY, ies, ies_len);
	if (cap_element) {
		if (!cap_element->datalen)
			return -EINVAL;
		params->eht_cap = (void *)(cap_element->data + 1);
		if (!ieee80211_eht_capa_size_ok((const u8 *)params->he_cap,
						(const u8 *)params->eht_cap,
						cap_element->datalen - 1,
						true))
			return -EINVAL;
	}	
	cap_element = cfg80211_find_ext_elem(WLAN_EID_EXT_EHT_OPERATION, ies, ies_len);
	if (cap && cap_element) {
		if (!cap_element->datalen)
			return -EINVAL;
		params->eht_oper = (void *)(cap_element->data + 1);
...
static int nl80211_set_beacon(struct sk_buff *skb, struct genl_info *info)
{
	...
	if(err == 0){
		if (info->attrs[NL80211_ATTR_SSID]) {
			...
			memcpy(wdev->ssid, nla_data(info->attrs[NL80211_ATTR_SSID]), wdev->ssid_len);
		}
	}
	wdev_unlock(wdev);
...
static int nl80211_new_station(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (info->attrs[NL80211_ATTR_EHT_OPERATION]) {
		params.eht_oper_iwlwav =
			nla_data(info->attrs[NL80211_ATTR_EHT_OPERATION]);
	}
	if (info->attrs[NL80211_ATTR_OPMODE_NOTIF]) {
		params.opmode_notif_used = true;
		params.opmode_notif =
					 ...
					 struct genl_info *info)
{
	...
	/* Start Background radar detection if supported */
...
static void nl80211_handle_sub_band_dfs(struct wireless_dev *wdev, struct cfg80211_csa_settings *params)
{
	...
	if ((params->chandef.sb_dfs.sub_width == NL80211_SB_DFS_BW_20) ||
		...
		if (!wdev->chandef.sb_dfs.sub_width) {
			params->chandef.sb_dfs.width = wdev->chandef.width;
			params->chandef.sb_dfs.center_freq = wdev->chandef.center_freq1;
		}
		else {
			params->chandef.sb_dfs.width = wdev->chandef.sb_dfs.width;
			params->chandef.sb_dfs.center_freq = wdev->chandef.sb_dfs.center_freq;
		}
	}
}
			       ...
			       struct cfg80211_chan_def *chandef)
{
	...
	nl80211_ch_switch_notify(rdev, dev, chandef, GFP_ATOMIC,
			      ...
			      u64 color_bitmap, u8 *bssid, u8 cca_color)
{
	...
	if (cmd == NL80211_CMD_COLOR_CHANGE_STARTED &&
	    (nla_put_u8(msg, NL80211_ATTR_COLOR_CHANGE_COUNT, count) ||
